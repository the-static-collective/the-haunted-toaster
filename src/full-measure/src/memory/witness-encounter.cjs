...
async function appendWitnessEncounter({
  ...
  const witness = {
    ...
    disposition: memoryContext?.witnessDisposition ? structuredClone(memoryContext.witnessDisposition) : null,
    reToastAncestor: memoryContext?.reToastAncestor ? structuredClone(memoryContext.reToastAncestor) : null,
  };
