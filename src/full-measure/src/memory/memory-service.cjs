...
function createMemoryService({ rootProvider } = {}) {
  ...
  async function archiveRender(renderResult) {
    return archiveSuccessfulRender({ rootDir: await rootDir(), renderResult });
  }
  ...
  async function submitVerdict(config = {}) {
    return appendHumanVerdict({ ...config, rootDir: await rootDir() });
  }

  async function resolveReToastAncestor(receiptSha256) {
    const root = await rootDir();
    const artifact = await resolveArchivedArtifact({ rootDir: root, receiptSha256, kind: "score" });
    ...
    return { receiptSha256, scoreAddress: parsed.address, score: parsed.value };
  }

  async function resolveArtifact({ receiptSha256, kind }) {
    return resolveArchivedArtifact({ rootDir: await rootDir(), receiptSha256, kind });
  }
