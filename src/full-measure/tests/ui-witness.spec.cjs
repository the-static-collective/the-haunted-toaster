...
test("witness Video source and VSPantry", async ({ page }, testInfo) => {
  ...
  await expect(page.locator("#videoDigestOperator")).toHaveCount(0);
  await expect(page.locator("#videoSamplingPolicy")).toHaveCount(0);
  ...
  await expect(page.locator("#videoDropHint")).toHaveText("Optional · MP4 or WebM");
  await expect(page.locator("#videoDigestOperator")).toHaveCount(0);
  await expect(page.locator("#videoSamplingPolicy")).toHaveCount(0);
});
