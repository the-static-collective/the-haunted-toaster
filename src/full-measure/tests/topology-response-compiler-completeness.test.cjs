...
test("Mirrored Ring and Quad Mirror route openness into internal separation", () => {
  assert.match(compiled("mirrored-ring"), /scoreRingAr/);
  assert.match(compiled("mirrored-ring"), /scoreRingBr/);
  assert.match(compiled("quad-mirror"), /scoreQ1r/);
  assert.match(compiled("quad-mirror"), /scoreQ4r/);
});

test("Elastic Spine and Split Horizon use signed recoil/openness before the final whole-frame response", () => {
  assert.match(compiled("elastic-spine"), /scoreSpineResponsive/);
  assert.match(compiled("split-horizon"), /shapeHorizonTopResponsive/);
  assert.match(compiled("split-horizon"), /shapeHorizonBottomResponsive/);
});

test("Echo Tunnel moves nested planes independently with signed travel", () => {
  const source = compiled("echo-tunnel");
  assert.match(source, /\[shapeTunnelBm\]scale[^;]*crop=[^;]*t[^;]*\[shapeTunnelBmResponsive\]/s);
  assert.match(source, /\[shapeTunnelCi\]scale[^;]*crop=[^;]*t[^;]*\[shapeTunnelCiResponsive\]/s);
});
