...
const EXPECTED_STRUCTURES = Object.freeze(["scope", "ribs", "lattice", "facets", "torus", "folds", "voxels", "branches"]);
const EXPECTED_DYNAMICS = Object.freeze(["inertial", "wave", "orbital-decay", "snap", "oscillation", "seismic", "magnetic", "swarm", "whip", "advect"]);
...
test("ordinary six-up assigns deterministic inspectable primitive fields", () => {
  ...
  assert.ok(first.candidates.every((candidate) => { const field = candidate.scoreArtifact.score.primitiveField; return field && EXPECTED_STRUCTURES.includes(field.structure) && EXPECTED_DYNAMICS.includes(field.dynamics); }));
...
test("primitive renderer is a legacy no-op and compiles accepted evidence before stage0", () => {
  ...
  const graph = ["[wave]pad=640:360:0:0:color=black@0.0[waveFull]", "[spectral][waveFull]overlay=0:0:shortest=1[stage0]", "[stage0]ass=filename='text-overlay.ass':alpha=1,format=yuv420p[vout]"].join(";\n");
  ...
  const timeline = { baseState: { primitiveField: { structure: "ribs", dynamics: "magnetic" } }, primitiveField: { policyVersion: "primitive-field-coverage-v1", structure: "ribs", dynamics: "magnetic", structureCompiler: "structure-ribs-v1", dynamicsCompiler: "dynamics-magnetic-v1" } };
...
test("nested response changes Primitive Field motion without changing categorical identity", () => {
  const { applyPrimitiveFieldToGraph } = require("../src/render/primitive-field.cjs");
  const graph = ["[wave]pad=640:360:0:0:color=black@0.0[waveFull]", "[spectral][waveFull]overlay=0:0:shortest=1[stage0]"].join(";\n");
  function timeline(planSha256, second) {
    return {
      rendererPolicy: generation.MUTATION_LATTICE_RENDERER_POLICY, timebase: 1000, durationTicks: 2000,
      baseState: { topology: "spiral", primitiveField: { structure: "ribs", dynamics: "magnetic" } },
      ...
      nestedResponse: {
        policyVersion: "nested-response-contour-v1", granularity: "transient", knotCount: 2,
        knots: [
        ...
        ], meterEvidenceUsed: false, idleMotionPolicyVersion: "topology-idle-v1", sourceWitnessSha256: `witness-${planSha256}`, planSha256,
      },
    };
  }
