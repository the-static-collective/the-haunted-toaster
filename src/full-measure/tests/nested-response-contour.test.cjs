...
test("response witness and signed local contour are deterministic", () => {
  ...
  assert.ok(directions.includes(1), "expected a rising response arc");
  assert.ok(directions.includes(0), "expected a settled response state");
  assert.ok(directions.includes(-1), "expected a recoil response arc");
});
...
test("timeline attachment preserves ordinary patch accounting", () => {
  ...
  const after = generation.attachNestedResponse(before, {
    responseWitness,
    score: score({ temporalDensity: "transient" }),
  });
...
test("true silence records zero signal-driven excursion", () => {
  ...
  const plan = generation.resolveNestedResponse({
    responseWitness,
    score: score({ temporalDensity: "transient" }),
    timeline: timeline({ durationTicks: 3000 }),
  });
...
test("hysteresis prevents tiny alternating deviations from flipping direction each sample", () => {
  ...
  const plan = generation.resolveNestedResponse({
    responseWitness,
    score: score({ temporalDensity: "transient" }),
    timeline: timeline({ durationTicks: 7000 }),
  });
...
test("response witness rejects unsorted and non-finite measurements", () => {
  assert.throws(() => generation.deriveResponseWitness({
    energySamples: [{ time: 1, db: -20 }, { time: 0, db: -21 }],
    sections: [],
    durationSeconds: 2,
  }), /sorted/i);
  assert.throws(() => generation.deriveResponseWitness({
    energySamples: [{ time: 0, db: Number.NaN }],
    sections: [],
    durationSeconds: 1,
  }), /finite/i);
});
